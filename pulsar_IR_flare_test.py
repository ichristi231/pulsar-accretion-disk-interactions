...
for i in range(np.shape(timestable)[0]):
    f = (gamma_table > gamma_min) * (gamma_table < gamma_cool_1[i]) * (1 - (1 -
    b * gamma_table * timestable[i])**(particle_slope - 1))
    f[np.isnan(f)] = 0
    ...
    f = (gamma_cool_1[i] < gamma_table) * (gamma_table < gamma_min) * ((1 -
        (gamma_max / gamma_min)**(1 - particle_slope)) * (gamma_min / gamma_table)**(1 - particle_slope))
    f[np.isnan(f)] = 0
    particle_distribution[i] += f
    ...
    particle_distribution[i] *= (particle_distribution_norm * gamma_table**(
        -1 - particle_slope) / (b * (particle_slope - 1)))
###############################################################
###############################################################
